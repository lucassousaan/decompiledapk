...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1395plusAssignLRDsOJo(long duration) {
        long newReading;
        long longDelta = Duration.m1355toLongimpl(duration, getUnit());
        if (longDelta == Long.MIN_VALUE || longDelta == Long.MAX_VALUE) {
            double delta = Duration.m1352toDoubleimpl(duration, getUnit());
            double newReading2 = this.reading + delta;
            if (newReading2 > Long.MAX_VALUE || newReading2 < Long.MIN_VALUE) {
                m1394overflowLRDsOJo(duration);
            }
    ...
    private final void m1394overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + "ns is advanced by " + Duration.m1358toStringimpl(duration) + '.');
    }
}
