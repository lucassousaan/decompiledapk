        ...
        if (this.start.readyToSolve && this.end.readyToSolve) {
            ...
            if (this.dimension.resolved || this.dimensionBehavior != ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT || this.widget.mMatchConstraintDefaultWidth != 0 || this.widget.isInVerticalChain()) {
                ...
                if (this.dimension.resolved && this.start.targets.size() > 0 && this.end.targets.size() > 0) {
                    ...
                    int distance = (endPos2 - startPos2) - this.dimension.value;
                    this.start.resolve((int) (startPos2 + 0.5f + (distance * bias)));
                    this.end.resolve(this.start.value + this.dimension.value);
                    return;
                }
                return;
            }
            int startPos3 = this.start.targets.get(0).value + this.start.margin;
            int endPos3 = this.end.targets.get(0).value + this.end.margin;
            int distance2 = endPos3 - startPos3;
            this.start.resolve(startPos3);
            this.end.resolve(endPos3);
            this.dimension.resolve(distance2);
        }
    }
